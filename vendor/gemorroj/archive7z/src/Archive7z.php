...
class Archive7z
{
    ...
    public function setOutputDirectory(string $directory): self
    {
        ...
        if (false === $outputDirectory) {var_dump($directory);
    ...
    protected function makeProcess(string $command, array $arguments): Process
    {
    ...
    protected function decorateCmdExtract(): array
    {
        $out = [];
        $out[] = '-y';
  ...
  { "utf-16be", MY__CP_UTF16BE }
    ...
    {
        $process = $this->makeProcess('e', \array_merge([
            $this->overwriteMode,
            '-o'.$this->outputDirectory,
